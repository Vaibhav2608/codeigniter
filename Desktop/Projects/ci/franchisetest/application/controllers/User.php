...
class User extends CI_Controller
{
    ...
    public function customer_form($param1="", $param2=""){
        ...
        if ($param1 == 'add_customer_form') {
            $page_data['page_name'] = 'customer_add';
            $page_data['page_title'] = get_phrase('Customer Add');
            $this->load->view('backend/index', $page_data);
        }elseif ($param1 == 'edit_franchise_form') {
            $page_data['page_name'] = 'franchise_edit';
            $page_data['user_id'] = $param2;
            $page_data['page_title'] = 'Edit Franchise';
            $this->load->view('backend/index', $page_data);
        }
        elseif($param1 == 'customer_invoice'){
            $page_data['page_name']='customer_invoice';
            $page_data['page_title'] = 'Customer Invoive';
            $page_data['customers'] = $this->user_model->get_franchise_customer($this->session->userdata('user_id'))->result_array();
            $this->load->view('backend/index', $page_data);
        }
        elseif($param1 == 'invoices'){
            $page_data['page_name'] = 'invoices';
            $page_data['page_title'] = 'Customer Invoices';
            $page_data['data'] = $this->user_model->get_invoice($this->session->userdata('user_id'))->result_array();
            $this->load->view('backend/index', $page_data);

        }elseif($param1 == 'view_invoice'){
            $page_data['page_name'] = 'view_invoice';
            $page_data['page_title'] = 'Invoice';
            $page_data['invoice_id'] = $param2;
            // $data = $this->user_modal->get_invoice_by_id($param2)->result_array();
            // $page_data['data'] = ;
            $this->load->view('backend/index', $page_data);
        }elseif($param1 == 'itr_form'){
            $page_data['page_name'] = 'itr_from';
            $page_data['page_title'] = 'ITR Form';
            $this->load->view('backend/index', $page_data);
        }elseif($param1 == 'view_itr_form'){
            $page_data['page_name'] = 'view_itr_form';
            $page_data['page_title'] = 'ITR Form';
            $page_data['data'] = $this->user_model->get_application_data('itr', $this->session->userdata('user_id'))->result_array();
            $this->load->view('backend/index', $page_data);
        }elseif($param1 == 'dsc_form'){
            $page_data['page_name'] = 'dsc_form';
            $page_data['page_title'] = 'Digital Sginature Form';
            $this->load->view('backend/index', $page_data);
        }elseif($param1 == 'view_dsc_form'){
            $page_data['page_name'] = 'view_dsc_form';
            $page_data['page_title'] = 'Digital Signature Form';
            $page_data['data'] = $this->user_model->get_application_data('dsc', $this->session->userdata('user_id'))->result_array();
            $this->load->view('backend/index', $page_data);
        }elseif($param1 == 'gst_form'){
            $page_data['page_name'] = 'gst_form';
            $page_data['page_title'] = 'Apply For GST';
            $this->load->view('backend/index', $page_data);
        }elseif($param1 == 'view_gstin_form'){
            $page_data['page_name'] = 'view_gst_form';
            $page_data['page_title'] = 'GSTIN Form';
            $page_data['data'] = $this->user_model->get_application_data('gst', $this->session->userdata('user_id'))->result_array();
            $this->load->view('backend/index', $page_data);
        }
    ...
    public function form($param1="", $param2=""){
        ...
        if($param1=='view_itr_form'){
            $page_data['page_name'] = 'applied_itr_form';
            $page_data['page_title'] = 'Applied ITR Form';
            $page_data['form_id'] = $param2;
            $this->load->view('backend/index', $page_data);
        }elseif($param1=='itr_form'){
            $this->user_model->itr_service_form();
            redirect(site_url('user/customer_form/view_itr_form'));
        }elseif($param1=='dsc_form'){
            $this->user_model->dsc_service_form();
            redirect(site_url('user/customer_form/view_itr_form'));
        }elseif($param1=='gst_form'){
            $this->user_model->gst_service_form();
            redirect(site_url('user/customer_form/view_itr_form'));
        }elseif($param1=='view_dsc_form'){
            $page_data['page_name'] = 'applied_dsc_form';
            $page_data['page_title'] = 'Applied DSC Form';
            $page_data['form_id'] = $param2;
            $this->load->view('backend/index', $page_data);
        }
        elseif($param1=='view_gst_form'){
            $page_data['page_name'] = 'applied_gst_form';
            $page_data['page_title'] = 'Applied GST Form';
            $page_data['form_id'] = $param2;
            $this->load->view('backend/index', $page_data);
        }
    ...
    public function get_customer(){
        ...
        $customer = $this->user_model->get_customer_by_invoice_id($invoice_id)->row();
        if($this->session->userdata('user_id')!=$customer->franchise_id){
            echo json_encode("wrong invoice number");
        }else{
            echo json_encode($customer);
        }
    }

    public function download_file($param1="", $param2=""){

        if(!empty($param1)){

            $this->load->helper('download');

            $file_url = $this->user_model->get_attachement_url($param1);
            $filename = 'application.zip';
            $data = file_get_contents("modulos/".$filename); // Read the file's contents
            force_download($filename, $data); 
            
            force_download('application.zip', NULL);
        }
    ...
    public function customer($param1="", $param2=""){
        if($param1 == 'add'){
            $this->user_model->add_customer();
            redirect(site_url('user/customers'), 'refresh');
        }
        if($param1 == 'add_invoice'){
            $this->user_model->add_invoice();
            redirect(site_url('user/customers'), 'refresh');
        }
        ...
        $page_data['page_title'] = 'List of Customer';
        $page_data['franchises'] = $this->user_model->get_franchise_customer($this->session->userdata('user_id'))->result_array();
        $this->load->view('backend/index', $page_data);
    }
